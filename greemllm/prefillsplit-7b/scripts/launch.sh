...
routing_config=${PREFILL_SPLIT_ROUTING_CONFIG_FILE:-"$experiment_root/config/routing.json"}

if [[ ! -s "$routing_config" ]]; then
  echo "Routing config is not available: $routing_config" >&2
  exit 1
fi
case "$routing_config" in
  "$experiment_root"/*) ;;
  *)
    echo "Routing config must be inside $experiment_root: $routing_config" >&2
    exit 1
    ;;
esac
routing_config_container=/experiment/${routing_config#"$experiment_root"/}
...
docker run --rm -d \
  ...
  -e PREFILL_SPLIT_ROUTING_CONFIG="$routing_config_container" \
