...
profile_gpus=0,1,2,3
config_path=${1:-$profile_root/config/alibaba_1qps_full_decode.yaml}
config_path=$(realpath "$config_path")
run_stamp=$(date -u +%Y%m%dT%H%M%SZ)
output=${2:-$profile_root/results/decode-$run_stamp}
output=$(realpath -m "$output")

reset_clocks() {
  if docker exec "$container" /lib64/ld-linux-x86-64.so.2 \
      /usr/local/bin/nvidia-smi -i "$profile_gpus" -rgc >/dev/null 2>&1; then
    return 0
  fi
  docker run --rm --pull never --gpus all --runtime nvidia --privileged \
    ...
    /usr/local/bin/nvidia-smi -i "$profile_gpus" -rgc >/dev/null 2>&1 || true
}
...
python3 "$profile_root/scripts/decode_profile.py" --config "$config_path" --output "$output"
reset_clocks
sleep 5
