...
# Keep the two locally-built images distinct: RTX 3090 uses SM86 and RTX 4090
# uses SM89.  Refuse a partial or mixed allocation instead of silently
# starting a configuration that cannot reproduce the eight-GPU baseline.
expected_gpu_count=8
if ! command -v nvidia-smi >/dev/null 2>&1; then
  echo "nvidia-smi is required to select the local GPU build" >&2
  exit 1
fi
...
echo "Started $container_name. Follow /experiment/logs/dynamo.log for readiness."
